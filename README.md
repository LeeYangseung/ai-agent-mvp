...
flowchart TB
    ...
    subgraph Backend["Backend (FastAPI)"]
        ...
        subgraph Core["Core Components"]
            subgraph GR["Graph Runner<br/>(LangGraph Executor)"]
            ...
            subgraph Utils["Utils"]
                CU[Chunking Utils<br/>Length/Semantic<br/>Hybrid/Paragraph]
                VS[Vector Store Utils<br/>Chroma]
            end
        end
    end
    
    subgraph DB["Data Storage"]
        PG[(PostgreSQL<br/>메타데이터<br/>─────<br/>Graphs<br/>Nodes<br/>Edges<br/>Documents<br/>Chunks<br/>Collections<br/>Graph History)]
        CB[(ChromaDB<br/>벡터 저장소<br/>─────<br/>Embeddings<br/>Collections<br/>Vector Index)]
    end
    
    subgraph K8s["Infrastructure (Kubernetes)"]
        K8S_INFO["• Deployments: Backend, Frontend, PostgreSQL<br/>• Services: ClusterIP, LoadBalancer<br/>• Ingress: Nginx Ingress Controller<br/>• ConfigMaps: 환경 변수 관리<br/>• Secrets: 민감 정보 관리<br/>• Persistent Volumes: 데이터 영속화"]
    end
