...
graph TB
    ...
    subgraph Frontend["Frontend (Next.js 15)"]
        GE[그래프 에디터<br/>React Flow]
        AM[Agent 관리<br/>목록/검색]
        KM[지식 관리 RAG<br/>컬렉션/문서/청크]
    end
    
    subgraph Backend["Backend (FastAPI)"]
        API[API Layer v1<br/>graphs, documents,<br/>collections, chunks]
        
        Service[Service Layer<br/>Graph, Node, Edge,<br/>Document, Collection, Chunk]
        
        subgraph Core["Core Components"]
            GR[Graph Runner<br/>LangGraph Executor]
            
            Nodes[Node Implementations<br/>Input, Prompt, Retrieval,<br/>Condition, Merge, Output]
            
            Utils[Utils<br/>Chunking: Length/Semantic/Hybrid/Paragraph<br/>Vector Store: Chroma]
        end
    end
    
    subgraph DB["Data Storage"]
        PG[(PostgreSQL<br/>메타데이터<br/>Graphs, Nodes, Edges,<br/>Documents, Chunks,<br/>Collections, History)]
        CB[(ChromaDB<br/>벡터 저장소<br/>Embeddings,<br/>Collections,<br/>Vector Index)]
    end
    
    subgraph K8s["Infrastructure - Kubernetes"]
        K8S_INFO[Deployments: Backend, Frontend, PostgreSQL<br/>Services: ClusterIP, LoadBalancer<br/>Ingress: Nginx<br/>ConfigMaps & Secrets<br/>Persistent Volumes]
    end
    ...
    K8s -.->|배포 및 관리| Frontend
    K8s -.->|배포 및 관리| Backend
    K8s -.->|배포 및 관리| DB
    
    style Frontend fill:#e1f5ff,stroke:#0066cc,stroke-width:2px
    style Backend fill:#fff4e1,stroke:#cc8800,stroke-width:2px
    style DB fill:#f0e1ff,stroke:#8800cc,stroke-width:2px
    style K8s fill:#e1ffe1,stroke:#00cc44,stroke-width:2px
    style Core fill:#ffe1e1,stroke:#cc0000,stroke-width:2px
```
